...
def get_address(address_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    return address_service.get_address(str(user.id), address_id, db)
...
def delete_address(address_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    address_service.delete_address(str(user.id), address_id, db)
